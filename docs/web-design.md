...
- `lg`（≥1024px）及以上：索引栏以独立圆角浮层常驻左侧，按常用入口、归档、回收站、分类和标签分组。
- `lg` 以下：索引栏收进抽屉，从顶栏按钮唤起，选完分类/标签自动收起。

### 分类与标签操作

- 分类是可展开/收起的树形索引，位于标签区上方；节点带计数，点击进入对应分类视图（含子分类），分类区右上角提供「管理分类」入口。分类与标签筛选可叠加，二者互不重置。
