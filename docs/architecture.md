...
Web 首屏（启动台）只请求设置（搜索引擎）与置顶书签（上限 100）；工作区按当前筛选（默认分类等）一次取完全部匹配书签（游标循环，单页 100 条）并加载标签；添加/编辑表单按需加载，管理后台整体及每个 tab（含导入导出）都是独立路由 chunk，进入对应页面时才加载。扩展只通过 popup / 右键菜单创建书签，不下载或缓存书签数据。
