...
分类与标签共同组织书签：分类是可嵌套的粗粒度归属（每个书签一个分类，亦可归为未分类），标签是细粒度标注。首页展示常用入口，书签支持搜索、分类/标签筛选、归档、回收站、重复网址检查和导入导出。列表与搜索按游标分页，不在浏览器或 Worker 中加载全量数据。

Web 分两个界面：主工作区专注书签浏览与卡片操作；独立的「管理后台」页统一整理分类、标签与 Favicon 设置（含概览统计）。删除、归档、合并等危险操作均需二次确认。接口契约见 [API](./api.md)。
